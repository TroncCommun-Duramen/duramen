...
| 17 | 🔴 A14 — **Bureau : session restaurée sans en-tête `x-commune-code`** (`bureau/app.js`, IIFE d'init ligne ~889). Au rechargement de la page bureau, l'en-tête n'est pas renvoyé → RLS masque les extractions et les lots privés. Vérifié en base le 7 juil. : sans en-tête, `extractions` = `[]` et lots privés = `[]`. Conséquences : stock surestimé (aucune sortie déduite), grumes déjà extraites re-proposées à la sélection → risque de double extraction acceptée par la base. Fix : poser l'en-tête dans `bLancerApp()` au lieu des seuls chemins de connexion. | Feature (bureau) | **Haute** |
...
| 19 | 🟠 A16 — **Case « Partager avec toutes les communes » jamais réinitialisée** (mobile, modale « Nommer le lot ») : après un lot partagé, le lot suivant est pré-coché partagé à l'insu de l'agent. Contredit les mentions légales (« Aucune donnée n'est partagée par défaut »). Fix : décocher dans `nouvOuvrirModal()`. | Feature | Moyenne |
